...
public sealed partial class FLAutomation
{
	public enum MyType : byte
	{
		...
		MIDIProgram,
		Tempo,
		/// <summary>A generic MIDI CC exposed via one of the "MIDI Out" plugin's knobs (see <see cref="MIDIControllerNumber"/>).</summary>
		MIDICC,
	}
	...
	/// <summary>Only used when <see cref="Type"/> is <see cref="MyType.MIDICC"/>: the MIDI CC number (0-127) this automates.</summary>
	public byte MIDIControllerNumber;
	...
	public static FLColor3 GetDefaultColor(MyType type)
	{
		switch (type)
		{
			...
			case MyType.MIDICC: return new FLColor3(96, 126, 142);
		}
	...
	private void WriteAutomationConnection(EndianBinaryWriter w, ushort target)
	{
		...
		w.WriteUInt16(Index);
		w.WriteUInt32(0);
		w.WriteUInt16(MyTypeToConnectionType(Type, MIDIControllerNumber));
	...
	private static ushort MyTypeToConnectionType(MyType t, byte midiControllerNumber)
	{
		switch (t)
		{
			...
			// Confirmed against a real FL Studio 20.9.2 export with CC1/CC5/CC65 automation:
			// the "MIDI Out" plugin's generic CC knobs are addressed as 0x8000 | ccNumber.
			case MyType.MIDICC: return (ushort)(0x8000 | midiControllerNumber);
		}
		throw new ArgumentOutOfRangeException(nameof(t), t, null);
	}
