...
internal sealed partial class TrackData
{
	...
	/// <summary>
	/// "Extra" MIDI CCs (Modulation, Portamento, etc.) that FL Studio's "MIDI Out" plugin exposes as
	/// generic knobs. A CC that only ever has one value gets baked in as a constant knob value
	/// (<see cref="GetExtraCCsForFLP"/>); one that changes more than once gets a real automation clip
	/// (<see cref="GetExtraCCAutomations"/>).
	/// </summary>
	private static readonly ControllerType[] ExtraCCTypes =
	{
		ControllerType.ModulationWheel,
		ControllerType.BreathController,
		ControllerType.FootController,
		ControllerType.PortamentoTime,
		ControllerType.Effects1Depth,
		ControllerType.Effects3Depth,
		ControllerType.SustainToggle,
		ControllerType.PortamentoToggle,
		ControllerType.SostenutoToggle,
		ControllerType.SoftPedalToggle,
		ControllerType.LegatoToggle,
		ControllerType.Hold2Toggle,
	};
	private readonly Dictionary<ControllerType, List<IMIDIEvent<ControllerMessage>>> _extraCCEventsOptimized;

	/// <summary>Maps each extra CC's <see cref="ControllerType"/> to its real MIDI CC number and a short display name.</summary>
	public static readonly Dictionary<ControllerType, (byte CCNumber, string Name)> ExtraCCInfo = new()
	{
		[ControllerType.ModulationWheel] = (1, "Modulation"),
		[ControllerType.BreathController] = (2, "Breath"),
		[ControllerType.FootController] = (4, "Foot Controller"),
		[ControllerType.PortamentoTime] = (5, "Portamento time"),
		[ControllerType.Effects1Depth] = (91, "Reverb send"),
		[ControllerType.Effects3Depth] = (93, "Chorus send"),
		[ControllerType.SustainToggle] = (64, "Sustain"),
		[ControllerType.PortamentoToggle] = (65, "Portamento"),
		[ControllerType.SostenutoToggle] = (66, "Sostenuto"),
		[ControllerType.SoftPedalToggle] = (67, "Soft pedal"),
		[ControllerType.LegatoToggle] = (68, "Legato"),
		[ControllerType.Hold2Toggle] = (69, "Hold 2"),
	};

	/// <summary>
	/// Returns the (ticks, value) events for each extra CC that changed value more than once in this
	/// track (i.e. needs a real automation clip rather than a constant knob value).
	/// </summary>
	public IEnumerable<(ControllerType Type, List<IMIDIEvent<ControllerMessage>> Events)> GetExtraCCAutomations()
	{
		foreach (KeyValuePair<ControllerType, List<IMIDIEvent<ControllerMessage>>> kvp in _extraCCEventsOptimized)
		{
	...
	public TrackData(byte trackIndex, MIDITrackChunk inTrack, byte defaultMIDIVol, Splitter split)
	{
		...
		_extraCCEventsOptimized = new Dictionary<ControllerType, List<IMIDIEvent<ControllerMessage>>>();
		foreach (ControllerType t in ExtraCCTypes)
		{
			_extraCCEventsOptimized[t] = new List<IMIDIEvent<ControllerMessage>>();
		}
	...
	private byte GatherTrackInfoFirstPass(byte defaultMIDIVol, Splitter split)
	{
		...
		var curExtraCC = new Dictionary<ControllerType, byte>();
		foreach (ControllerType t in ExtraCCTypes)
		{
			curExtraCC[t] = 0; // GM default for all of these is 0
		}
			...
			switch (ev)
			{
				...
				case IMIDIEvent<ControllerMessage> e:
				{
					...
					switch (e.Msg.Controller)
					{
						...
						default:
						{
							if (curExtraCC.ContainsKey(e.Msg.Controller) && curExtraCC[e.Msg.Controller] != newVal)
							{
								_extraCCEventsOptimized[e.Msg.Controller].Add(e);
								curExtraCC[e.Msg.Controller] = newVal;
							}
							break;
						}
					}
	...
	/// <summary>
	/// Returns a constant (non-automated) value for each "extra" MIDI CC (Modulation, Portamento, etc.)
	/// that was set exactly once (at tick 0) in this track, for baking into the FLP's "MIDI Out" generic
	/// knobs. CCs that changed value more than once are left out here; <see cref="GetExtraCCAutomations"/>
	/// turns those into real automation clips instead.
	/// </summary>
	public MIDIOutExtraCCs GetExtraCCsForFLP()
	{
		var result = new MIDIOutExtraCCs();
		foreach (KeyValuePair<ControllerType, List<IMIDIEvent<ControllerMessage>>> kvp in _extraCCEventsOptimized)
		{
			if (kvp.Value.Count != 1)
			{
				continue; // 0 => never set, leave FL's default. >1 => handled as automation instead.
			}
			byte val = kvp.Value[0].Msg.Value;
			switch (kvp.Key)
			{
				case ControllerType.ModulationWheel: result.Modulation = val; break;
				case ControllerType.BreathController: result.Breath = val; break;
				case ControllerType.FootController: result.Foot = val; break;
				case ControllerType.PortamentoTime: result.PortamentoTime = val; break;
				case ControllerType.Effects1Depth: result.ReverbSend = val; break;
				case ControllerType.Effects3Depth: result.ChorusSend = val; break;
				case ControllerType.SustainToggle: result.Sustain = val; break;
				case ControllerType.PortamentoToggle: result.Portamento = val; break;
				case ControllerType.SostenutoToggle: result.Sostenuto = val; break;
				case ControllerType.SoftPedalToggle: result.SoftPedal = val; break;
				case ControllerType.LegatoToggle: result.Legato = val; break;
				case ControllerType.Hold2Toggle: result.Hold2 = val; break;
			}
		}
		return result;
	}
