...
{
	public void FLP_AddNewTracks(FLPSaver saver, Splitter split)
	{
		...
		List<FLChannel> ourChans = FLP_CreateChannels(saver, f, dict, split);
		FLP_CreatePatterns(saver, shortName, ourChans, dict);
		FLP_CreateAutomations(saver, f, ourChans);
	}
	private List<FLChannel> FLP_CreateChannels(FLPSaver saver, FLChannelFilter filter, Dictionary<MIDIProgram, NewTrack> dict, Splitter split)
	{
		...
		MIDIOutExtraCCs extraCCs = GetExtraCCsForFLP();
	...
	private void FLP_CreateAutomations(FLPSaver saver, FLChannelFilter filter, List<FLChannel> ourChans)
	{
		...
		HandleExtraCCEvents(saver, filter, ourChans);
	}
	...
	private void HandleExtraCCEvents(FLPSaver saver, FLChannelFilter filter, List<FLChannel> ourChans)
	{
		foreach ((ControllerType type, List<IMIDIEvent<ControllerMessage>> events) in GetExtraCCAutomations())
		{
			(byte ccNumber, string name) = ExtraCCInfo[type];
			string label = string.Format("{0} (CC{1})", name, ccNumber);

			FLAutomation a = CreateAuto(saver, label, FLAutomation.MyType.MIDICC, filter, ourChans);
			a.MIDIControllerNumber = ccNumber;
			foreach (IMIDIEvent<ControllerMessage> e in events)
			{
				a.AddPoint((uint)e.Ticks, ExtraCCToAutomation(e.Msg.Value));
			}
			saver.AddMIDITrackAuto(a, ExtraCCToAutomation(0)); // GM default for all of these is 0
		}
	}
	private static double ExtraCCToAutomation(byte val)
	{
		return val / 127d;
	}
